...
try{
    $customer=customer_auth_require();$data=customer_api_json();customer_push_unsubscribe((int)$customer['id'],trim((string)($data['endpoint']??'')));customer_api_reply(200,['ok'=>true]);
...
catch(Throwable $e){customer_api_reply(500,['ok'=>false,'error'=>'Не удалось отключить push-уведомления.']);}

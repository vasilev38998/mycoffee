...
// customer_order_public_status() may synchronously change a YooKassa order from
// awaiting_payment to new/cancelled. Always reload after that sync so loyalty,
// Evotor push and the response all act on one current database state.
if(preg_match('/^[a-f0-9]{64}$/',$token)){
    ...
    if($orderId>0&&$status==='new'){
        try{evotor_order_notify_new($orderId);}catch(Throwable $pushError){error_log('[Kapouch Evotor push paid poll] '.$pushError->getMessage());}
    }
}

...
if(preg_match('/^[a-f0-9]{64}$/',$token)){
    $stmt=db()->prepare('SELECT o.id,o.promised_at,o.status FROM customer_order_access a JOIN online_orders o ON o.id=a.order_id WHERE a.tracking_token=? LIMIT 1');$stmt->execute([$token]);
    $row=$stmt->fetch()?:[];$promised=trim((string)($row['promised_at']??''));$order['promised_at']=$promised;$order['promised_display']=$promised!==''?date('H:i',strtotime($promised)):'';

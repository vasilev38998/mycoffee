...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        if($action==='business'){
            ...
            flash('success','Основные настройки сохранены. Система работает по времени Иркутска (UTC+8).');
        }
        if($action==='goals'){
...
?>
<div class="two-col">
<div class="card"><div class="chart-head"><div><h2>Кофейня</h2><p>Основные параметры системы</p></div></div><form method="post" class="form-grid"><input type="hidden" name="csrf" value="<?=csrf_token()?>"><input type="hidden" name="action" value="business"><label>Название кофейни<input name="coffee_name" value="<?=e((string)app_setting('coffee_name','MyCoffee'))?>" required></label><label>Часовой пояс<input value="Иркутск (UTC+8, +5 часов к Москве)" disabled><span class="muted">Зафиксирован, чтобы исторические и новые чеки всегда были в одном времени.</span></label><label>Валюта<input name="currency" value="<?=e(app_currency())?>" maxlength="8"></label><label>Открытие<input type="time" name="opening_hour" value="<?=e((string)app_setting('opening_hour','07:00'))?>"></label><label>Закрытие<input type="time" name="closing_hour" value="<?=e((string)app_setting('closing_hour','21:00'))?>"></label><div><button class="btn primary">Сохранить настройки</button></div></form></div>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verify_csrf();
    $ingredientId=(int)($_POST['ingredient_id']??0);$quantity=(float)($_POST['quantity']??0);$totalAmount=(float)($_POST['total_amount']??0);$supplierId=(int)($_POST['supplier_id']??0);$cashAccountId=(int)($_POST['cash_flow_account_id']??0);$purchasedAt=$_POST['purchased_at']??date('Y-m-d');
    if($ingredientId>0&&$quantity>0&&$totalAmount>=0){$pdo=db();$pdo->beginTransaction();try{$supplierName='';if($supplierId>0){$supplierStmt=$pdo->prepare('SELECT name FROM suppliers WHERE id=? AND active=1');$supplierStmt->execute([$supplierId]);$supplierName=(string)($supplierStmt->fetchColumn()?:'');if($supplierName==='')throw new RuntimeException('Поставщик не найден или отключён.');}$previousStmt=$pdo->prepare('SELECT total_amount/NULLIF(quantity,0) FROM purchases WHERE ingredient_id=? AND quantity>0 ORDER BY purchased_at DESC,id DESC LIMIT 1');$previousStmt->execute([$ingredientId]);$previousPrice=$previousStmt->fetchColumn();
        $stmt=$pdo->prepare('INSERT INTO purchases(purchased_at,ingredient_id,quantity,total_amount,supplier,supplier_id,cash_flow_account_id) VALUES(?,?,?,?,?,?,?)');$stmt->execute([$purchasedAt,$ingredientId,$quantity,$totalAmount,$supplierName,$supplierId?:null,$cashAccountId?:null]);$purchaseId=(int)$pdo->lastInsertId();
        $update=$pdo->prepare('UPDATE ingredients SET stock_quantity=stock_quantity+?,purchase_price=?,purchase_quantity=? WHERE id=?');$update->execute([$quantity,$totalAmount,$quantity,$ingredientId]);
        $movement=$pdo->prepare('INSERT INTO inventory_movements(ingredient_id,movement_type,quantity_delta,reference_type,reference_id,occurred_at,note) VALUES(?,?,?,?,?,?,?)');$movement->execute([$ingredientId,'purchase',$quantity,'purchase',$purchaseId,$purchasedAt.' 12:00:00',$supplierName!==''?'Закупка: '.$supplierName:'Закупка']);
        ...
        if($cashAccountId>0&&$totalAmount>0)cashflow_add_entry(['occurred_at'=>$purchasedAt.' 12:00:00','account_id'=>$cashAccountId,'direction'=>'out','entry_type'=>'purchase','amount'=>$totalAmount,'source_type'=>'purchase','source_id'=>(string)$purchaseId,'category'=>'Закупки','description'=>$supplierName!==''?'Закупка у '.$supplierName:'Закупка ингредиента']);
        $pdo->commit();$newPrice=$totalAmount/$quantity;$change=$previousPrice!==false&&(float)$previousPrice>0?($newPrice-(float)$previousPrice)/(float)$previousPrice*100:null;audit_write('purchase_created','Добавлена закупка','purchase',(string)$purchaseId,['ingredient_id'=>$ingredientId,'quantity'=>$quantity,'total_amount'=>$totalAmount,'supplier_id'=>$supplierId?:null,'cash_flow_account_id'=>$cashAccountId?:null,'unit_price'=>$newPrice,'price_change_pct'=>$change]);$message='Закупка сохранена, остаток и закупочная цена обновлены.';if($cashAccountId>0)$message.=' Денежный расход учтён.';if($change!==null&&$change>=(float)app_setting('purchase_price_warning_pct','10'))$message.=' Цена выросла на '.number_format($change,1,',',' ').'%. Проверь раздел «Закупочные цены».';flash('success',$message);

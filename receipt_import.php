...
$id=(int)($_GET['id']??0);$draft=$id?receipt_draft($id):null;
if($draft&&$draft['status']==='draft'){receipt_package_reconcile_draft($id);$draft=receipt_draft($id);}
...
$connection=receipt_connection();$recent=db()->query("SELECT id,receipt_at,seller_name,total_amount,status,source FROM purchase_receipts ORDER BY id DESC LIMIT 20")->fetchAll();
...
<div class="two-col section">
  <div class="card"><h2>1. Сканировать QR</h2><p>После получения состава Kapouch автоматически попробует определить объём или вес упаковки из названия товара.</p><form method="post" id="qrForm" class="form-grid"><input type="hidden" name="csrf" value="<?=csrf_token()?>"><input type="hidden" name="action" value="scan_qr"><label>QR-код<input id="qrRaw" name="qr_raw" required placeholder="t=...&s=...&fn=...&i=...&fp=...&n=1"></label><label>Фото QR<input id="qrImage" type="file" accept="image/*" capture="environment"></label><div><button class="btn primary">Получить чек</button></div></form><div id="qrHint" class="muted" style="margin-top:10px">Если браузер поддерживает BarcodeDetector, QR с фото заполнится автоматически.</div></div>
  <div class="card"><h2>Резервный JSON</h2><form method="post" class="form-grid"><input type="hidden" name="csrf" value="<?=csrf_token()?>"><input type="hidden" name="action" value="import_json"><label>QR, если есть<input name="qr_raw"></label><label>JSON чека<textarea name="receipt_json" rows="9" required></textarea></label><div><button class="btn primary">Разобрать чек</button></div></form></div>
</div>
<div class="card section"><div class="chart-head"><div><h2>Источник электронных чеков</h2><p>Текущий провайдер используется для получения полного состава по QR.</p></div><span class="pill <?=$connection?'connected':''?>"><?=$connection?'Подключено':'Не настроено'?></span></div><form method="post" class="form-grid"><input type="hidden" name="csrf" value="<?=csrf_token()?>"><input type="hidden" name="action" value="save_connection"><label>Название<input name="connection_name" value="<?=e((string)($connection['name']??'Электронные чеки'))?>"></label><label>HTTPS endpoint<input type="url" name="endpoint_url" value="<?=e((string)($connection['endpoint_url']??''))?>" required></label><label>Bearer token<input type="password" name="token" autocomplete="new-password" placeholder="<?=$connection?'Оставь пустым, чтобы не менять':'Токен источника'?>"></label><div><button class="btn ghost">Сохранить интеграцию</button></div></form></div>
<?php else:?>
...
<tr class="receipt-row <?=!$row['included']?'excluded':''?>"><td><label class="receipt-include"><input type="checkbox" name="included[<?=$rid?>]" value="1" <?=$row['included']?'checked':''?>><span><strong><?=e($row['raw_name'])?></strong><small><?=e((string)$row['receipt_quantity'])?> × <?=money((float)$row['unit_price'])?> = <?=money((float)$row['line_total'])?></small></span></label><?php if(!empty($row['package_warning'])):?><div class="alert warning" style="margin-top:7px;padding:7px 9px"><?=e($row['package_warning'])?></div><?php endif;?></td><td><?php if($pack):?><span class="pill connected"><?=e($pack)?></span><?php else:?><span class="pill">Не найдено</span><?php endif;?></td><td><select name="ingredient[<?=$rid?>]"><option value="0">— выбрать —</option><?php foreach($ingredients as $i):?><option value="<?=$i['id']?>" <?=((int)$row['ingredient_id']===(int)$i['id'])?'selected':''?>><?=e($i['name'])?> · <?=e($i['unit'])?></option><?php endforeach;?></select><?php if($row['ingredient_id']&&$row['package_signature']&&!$row['package_warning']):?><div class="muted">✓ вариант упаковки распознан</div><?php endif;?></td><td><input type="number" step="0.001" min="0" name="per_item[<?=$rid?>]" value="<?=e((string)($row['quantity_per_item']??''))?>" placeholder="1000 / 1500 / 930"><div class="muted">в единицах ингредиента: г / мл / шт.</div></td><td><label style="display:flex;gap:7px;align-items:center"><input type="checkbox" name="save_rule[<?=$rid?>]" value="1" style="width:auto"> Запомнить этот размер</label></td><td><button type="button" class="btn ghost exclude-btn" data-exclude="<?=$rid?>"><?=$row['included']?'Не для кофейни':'Вернуть'?></button></td></tr>
<?php endforeach;?></tbody></table></div></div>
<div class="card section"><div class="form-grid"><label>Денежный счёт<select name="cash_flow_account_id"><option value="0">Не учитывать в Cash Flow</option><?php foreach($accounts as $a):?><option value="<?=$a['id']?>"><?=e($a['name'])?></option><?php endforeach;?></select></label><div class="actions" style="align-self:end"><button class="btn ghost" type="submit" onclick="document.getElementById('draftAction').value='save_draft'">Сохранить</button><button class="btn primary" type="submit" onclick="document.getElementById('draftAction').value='commit'">Оприходовать на склад</button></div></div></div>
</form>
<?php endif;?>
<?php endif;?>

<div class="card section table-card"><div class="chart-head"><div><h2>Последние чеки</h2><p>Повторно оприходовать один и тот же фискальный чек нельзя.</p></div></div><table><thead><tr><th>Дата</th><th>Поставщик</th><th>Сумма</th><th>Статус</th><th></th></tr></thead><tbody><?php foreach($recent as $r):?><tr><td><?=e($r['receipt_at']?date('d.m.Y H:i',strtotime($r['receipt_at'])):'—')?></td><td><?=e($r['seller_name']?:'—')?></td><td><?=money((float)$r['total_amount'])?></td><td><?=e($r['status'])?></td><td><a class="btn ghost" href="receipt_import.php?id=<?=$r['id']?>">Открыть</a></td></tr><?php endforeach;?></tbody></table></div>
<script>
(function(){

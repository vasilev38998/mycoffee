...
final class LegacyTls {
    ...
    // Old Evotor Android builds predate current Let's Encrypt trust anchors.
    // We keep the platform trust store first, then add only official ISRG/Let's Encrypt roots.
    // Hostname verification stays the HttpsURLConnection platform default and is never disabled.
    ...
    private static String chainSummary(X509Certificate[] chain) {
        if (chain == null || chain.length == 0) return "пустая цепочка";
        StringBuilder result = new StringBuilder();
        for (int i = 0; i < chain.length; i++) {
            if (i > 0) result.append(" -> ");
            result.append(chain[i].getSubjectX500Principal().getName());
        }
        ...
        public void checkServerTrusted(X509Certificate[] chain, String authType) throws CertificateException {
            try {
                ...
                try {
                    ...
                    return;
                } catch (CertificateException extraError) {
                    throw new CertificateException("TLS chain not trusted: " + chainSummary(chain), extraError);
                }
            }
        }

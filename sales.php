...
<div class="card section"><h2>Последние продажи</h2><table><thead><tr><th>Дата</th><th>Позиция</th><th>Кол-во</th><th>Выручка</th><th>Себестоимость</th><th>Валовая прибыль</th></tr></thead><tbody><?php foreach($rows as $r):$rev=(float)$r['unit_price']*(float)$r['quantity'];$cost=(float)$r['unit_cost']*(float)$r['quantity'];?><tr><td><?=e(date('d.m.Y H:i',strtotime($r['sold_at'])))?></td><td><?=e($r['name'])?></td><td><?=e((string)$r['quantity'])?></td><td><?=money($rev)?></td><td><?=money($cost)?></td><td><?=money($rev-$cost)?></td></tr><?php endforeach;?></tbody></table></div>
<?php page_footer(); ?>
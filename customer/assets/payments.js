...
function render(){
  ...
  box.innerHTML='<div class="payment-methods-title">Способ оплаты</div><div class="payment-method-list">'+methods.map((m,i)=>'<label class="payment-method"><input type="radio" name="payment_method" value="'+esc(m.id)+'" '+(i===0?'checked':'')+'><span class="payment-method-copy"><strong>'+esc(m.label)+'</strong><span>'+(m.id==='sbp'?'Оплата онлайн через СБП · чек придёт на email из профиля':'Оплата при получении заказа')+'</span></span></label>').join('')+'</div>'+(methods.some(m=>m.id==='sbp')?'<div class="payment-online-note">Для оплаты по СБП нужен email в профиле Kapouch. Заказ попадёт бариста только после подтверждения ЮKassa.</div>':'');
}
...
async function submit(e){
  ...
  try{
    ...
    if(method==='sbp'){
      if(!order.payment_url)throw new Error('ЮKassa не вернула ссылку для оплаты.');

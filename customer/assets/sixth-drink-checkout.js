...
function render(q){const box=ensureBox(),reward=q?.reward||{},gift=q?.gift||null,discount=Number(q?.discount||0);if(Number.isFinite(Number(q?.total)))totalEl.textContent=money(q.total);if(discount>0&&gift){box.innerHTML='<div class="sixth-drink-checkout-row"><span>Подарок «6-й напиток»</span><strong>−'+money(discount)+'</strong></div><small>'+String(gift.product_name||'Напиток')+' — скидка до '+money(gift.gift_cap||discount)+'. Если напиток дороже, оплачивается только разница; добавки оплачиваются отдельно.</small>';box.hidden=false;return}if(Number(reward.available_rewards||0)>0){box.innerHTML='<div class="sixth-drink-checkout-row"><span>Подарок доступен</span><strong>🎁</strong></div><small>Добавьте в корзину напиток из программы — скидка применится автоматически.</small>';box.hidden=false;return}clear()}

...
function showLogin(){$('loginCard').hidden=false;$('profileSection').hidden=true;$('codeForm').hidden=true;$('phoneForm').hidden=false;}
...
$('codeForm').addEventListener('submit',async e=>{
  ...
  try{const data=await api('customer_auth_verify.php',{method:'POST',body:JSON.stringify({phone,code:$('authCode').value.trim()})});token=data.auth.token;localStorage.setItem('kapouch_customer_auth_token',token);await loadProfile();}
...
$('logoutButton').addEventListener('click',async()=>{try{await api('customer_logout.php',{method:'POST'});}catch(e){}token='';localStorage.removeItem('kapouch_customer_auth_token');showLogin();});

function renderOrders(orders){$('ordersList').innerHTML=orders.length?orders.map(o=>'<a class="account-row" href="index.html"><div><strong>#'+esc(o.order_number)+'</strong><span>'+esc(o.status_label)+' · '+esc(String(o.external_created_at||o.created_at||'').slice(0,16).replace('T',' '))+'</span></div><b>'+money(o.total_amount)+'</b></a>').join(''):'<div class="empty">Заказов по этому номеру пока нет.</div>';}
function renderLoyalty(rows){$('loyaltyList').innerHTML=rows.length?rows.map(r=>'<div class="account-row"><div><strong>'+(r.operation_type==='earn'?'Начисление':'Операция')+'</strong><span>'+esc(r.note||'')+' · '+esc(String(r.created_at||'').slice(0,16))+'</span></div><b>'+((Number(r.amount)>=0)?'+':'')+money(r.amount)+'</b></div>').join(''):'<div class="empty">Пока нет операций с бонусами.</div>';}
async function loadProfile(){if(!token){showLogin();return;}try{const data=await api('customer_profile.php');const p=data.profile;showProfile();$('profileBalance').textContent=money(p.customer.loyalty_balance);$('profilePhone').textContent=p.customer.phone;$('profileName').textContent=p.customer.name||'Гость';renderOrders(p.orders||[]);renderLoyalty(p.loyalty||[]);}catch(err){token='';localStorage.removeItem('kapouch_customer_auth_token');showLogin();}}
loadProfile();
})();

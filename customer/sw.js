const CACHE='kapouch-pwa-v3';
const SHELL=['./','./index.html','./config.js?v=6','./assets/app.css?v=5','./assets/app.js?v=6','./assets/push.js?v=1','./assets/icon.svg'];
...
self.addEventListener('push',event=>{
  let data={title:'Kapouch',body:'У вас новое уведомление',url:'./'};
  try{if(event.data)data={...data,...event.data.json()}}catch(e){if(event.data)data.body=event.data.text()}
  event.waitUntil(self.registration.showNotification(data.title||'Kapouch',{
    body:data.body||'',icon:data.icon||'./assets/icon.svg',badge:data.badge||'./assets/icon.svg',tag:data.tag||'kapouch',renotify:true,data:{url:data.url||'./'},vibrate:[180,80,180]
  }));
});

...
function customer_payment_save_sber(array $data): void
{
    ...
    if($enabled&&!$test&&$apiBase==='')throw new RuntimeException('Для боевого режима укажите API Base URL, выданный Сбером.');
...
function customer_payment_create_sbp(int $orderId,string $orderNumber,float $amount,string $phone): array
{
    ...
    if($existing&&$existing['provider']==='sber_sbp'&&in_array((string)$existing['status'],['pending','paid'],true)&&!empty($existing['provider_order_id'])){
        return ['payment_url'=>(string)($existing['payment_url']?:$existing['sbp_payload']),'sbp_payload'=>(string)($existing['sbp_payload']??''),'provider_order_id'=>(string)$existing['provider_order_id'],'status'=>(string)$existing['status']];
    }
...
function customer_payment_sber_sync_by_provider_id(string $providerOrderId): ?array
{
    ...
    $stmt=db()->prepare("SELECT p.*,o.total_amount,o.status order_status FROM customer_payments p JOIN online_orders o ON o.id=p.order_id WHERE p.provider='sber_sbp' AND p.provider_order_id=? LIMIT 1");$stmt->execute([$providerOrderId]);$payment=$stmt->fetch();if(!$payment)return null;
    ...
    if($paid){
        ...
        db()->prepare("UPDATE online_orders SET status=CASE WHEN status='awaiting_payment' THEN 'new' ELSE status END,payment_status='paid' WHERE id=?")->execute([(int)$payment['order_id']]);
    }else{

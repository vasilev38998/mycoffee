...
function customer_pwa_guess_category(string $name): string
{
    ...
    if(str_contains($s,'чай'))return 'tea';if(str_contains($s,'лимонад'))return 'lemonades';if(str_contains($s,'коктейл')||str_contains($s,'милкшейк'))return 'milkshakes';if(str_contains($s,'выпеч')||str_contains($s,'круас')||str_contains($s,'десерт'))return 'bakery';return 'other';
}
...
{
    $path=trim((string)$path);if($path===''||!str_starts_with($path,'uploads/products/'))return null;return '../customer/'.$path;
}
function customer_pwa_catalog(): array
{
    ...
    $variantRows=db()->query("SELECT g.id group_id,g.name group_name,g.category_id group_category_id,g.description group_description,g.badge group_badge,g.image_path group_image_path,g.featured group_featured,g.visible group_visible,g.sort_order group_sort,
        v.product_id,v.variant_label,v.sort_order variant_sort,v.is_default,p.name product_name,p.category product_category,p.sale_price,p.active,s.visible product_visible,s.image_path product_image_path
        FROM customer_product_groups g JOIN customer_product_group_variants v ON v.group_id=g.id JOIN products p ON p.id=v.product_id LEFT JOIN customer_product_settings s ON s.product_id=p.id
        WHERE g.visible=1 AND p.active=1 AND p.sale_price>0 AND COALESCE(s.visible,1)=1 ORDER BY g.sort_order,g.id,v.is_default DESC,v.sort_order,p.sale_price,p.name")->fetchAll();
    $products=[];$groups=[];
    foreach($variantRows as $r){
        $gid=(int)$r['group_id'];$pid=(int)$r['product_id'];
        if(!array_key_exists($gid,$groups)){
            $cat=customer_pwa_category_for($r['group_category_id']?(int)$r['group_category_id']:null,(string)($r['product_category']?:$r['group_name']),$byId,$bySlug);if(!$cat){$groups[$gid]=null;continue;}
            $groups[$gid]=['key'=>'g'.$gid,'group_id'=>$gid,'id'=>$pid,'name'=>(string)$r['group_name'],'price'=>(float)$r['sale_price'],'description'=>trim((string)($r['group_description']??''))?:'Выберите подходящий объём.','badge'=>trim((string)($r['group_badge']??'')),'image'=>customer_pwa_image_url($r['group_image_path']??null),'featured'=>(bool)$r['group_featured'],'category_id'=>(int)$cat['id'],'category'=>(string)$cat['name'],'category_slug'=>(string)$cat['slug'],'category_icon'=>(string)$cat['icon'],'variants'=>[],'default_product_id'=>$pid,'sort_order'=>(int)$r['group_sort']];
        }
        if($groups[$gid]===null)continue;
        $variant=['id'=>$pid,'label'=>(string)$r['variant_label'],'price'=>(float)$r['sale_price'],'product_name'=>(string)$r['product_name'],'is_default'=>(bool)$r['is_default'],'image'=>customer_pwa_image_url($r['product_image_path']??null)];
        $groups[$gid]['variants'][]=$variant;
        if((bool)$r['is_default']||count($groups[$gid]['variants'])===1){$groups[$gid]['id']=$pid;$groups[$gid]['default_product_id']=$pid;$groups[$gid]['price']=(float)$r['sale_price'];if(!$groups[$gid]['image']&&$variant['image'])$groups[$gid]['image']=$variant['image'];}
    }
    ...
    foreach($rows as $r){
        $pid=(int)$r['id'];if(isset($groupedProductIds[$pid]))continue;$cat=customer_pwa_category_for($r['category_id']?(int)$r['category_id']:null,(string)($r['category']?:$r['name']),$byId,$bySlug);if(!$cat)continue;
        $image=customer_pwa_image_url($r['image_path']??null);

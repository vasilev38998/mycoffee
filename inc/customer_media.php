...
{
    $path=trim((string)$path);if($path===''||!str_starts_with($path,customer_media_public_prefix()))return;$file=dirname(__DIR__).'/customer/'.$path;if(is_file($file))@unlink($file);
}
function customer_media_save_upload(array $file,string $prefix,?string $oldPath=null): string
{
    ...
    $tmp=(string)($file['tmp_name']??'');if($tmp===''||!is_uploaded_file($tmp))throw new RuntimeException('Некорректный файл загрузки.');$info=@getimagesize($tmp);if(!$info)throw new RuntimeException('Файл не является изображением.');$iw=(int)($info[0]??0);$ih=(int)($info[1]??0);if($iw<1||$ih<1||$iw*$ih>36000000)throw new RuntimeException('Слишком большое разрешение фото. Максимум около 36 мегапикселей.');

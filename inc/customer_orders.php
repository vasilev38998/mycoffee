...
function customer_order_create(array $data): array
{
    ...
    $stmt=db()->prepare("SELECT p.id,p.name,p.sale_price
        FROM products p
        LEFT JOIN customer_product_settings cps ON cps.product_id=p.id
        LEFT JOIN customer_categories pc ON pc.id=cps.category_id
        LEFT JOIN customer_product_group_variants cgv ON cgv.product_id=p.id
        LEFT JOIN customer_product_groups cg ON cg.id=cgv.group_id
        LEFT JOIN customer_categories gc ON gc.id=cg.category_id
        WHERE p.active=1 AND p.sale_price>0
          AND COALESCE(cps.visible,1)=1
          AND (cps.category_id IS NULL OR COALESCE(pc.active,0)=1)
          AND (cgv.group_id IS NULL OR COALESCE(cg.visible,0)=1)
          AND (cg.category_id IS NULL OR COALESCE(gc.active,0)=1)

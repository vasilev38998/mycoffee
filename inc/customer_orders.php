...
function customer_order_create(array $data,?array $authenticatedCustomer=null): array
{
    ...
    $paymentMethod=customer_order_payment_method($data);$customerId=0;$email='';
    if($paymentMethod['id']==='sbp'){
        if(!$authenticatedCustomer)throw new RuntimeException('Для оплаты по СБП сначала войдите в профиль Kapouch.');$profilePhone=customer_order_normalize_phone((string)($authenticatedCustomer['phone']??''));
        ...
        $customerId=(int)($authenticatedCustomer['id']??0);if($customerId<=0)throw new RuntimeException('Не удалось определить профиль покупателя. Войдите заново.');$email=customer_order_account_email($customerId);if(!filter_var($email,FILTER_VALIDATE_EMAIL))throw new RuntimeException('Для оплаты по СБП сначала укажите электронную почту в профиле Kapouch.');$phone=$profilePhone;
    }
    ...
    }catch(Throwable $e){if($paymentMethod['id']==='sbp')db()->prepare("UPDATE online_orders SET status='cancelled',cancelled_at=NOW(),payment_status='failed',payment_method='sbp',payment_provider='yookassa_sbp' WHERE id=? AND status IN ('new','awaiting_payment')")->execute([$orderId]);throw $e;}

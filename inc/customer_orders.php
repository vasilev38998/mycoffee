...
{
    $name=mb_substr(trim($name),0,160);$stored=$name!==''?$name:null;
    $stmt=db()->prepare("INSERT INTO customer_accounts(phone,name) VALUES(?,?) ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id),name=IF(?='',name,?)");
    $stmt->execute([$phone,$stored,$name,$stored]);$id=(int)db()->lastInsertId();
    if($id<=0){$find=db()->prepare('SELECT id FROM customer_accounts WHERE phone=?');$find->execute([$phone]);$id=(int)($find->fetchColumn()?:0);}
    if($id<=0)throw new RuntimeException('Не удалось создать профиль покупателя.');return $id;
}
...
function customer_order_payment_method(array $data): array
{
    ...
    $requested=trim((string)($data['payment_method']??''));if($requested===''){if(count($enabled)===1)return $enabled[0];throw new RuntimeException('Выберите способ оплаты.');}
    foreach($enabled as $method)if($method['id']===$requested)return $method;throw new RuntimeException('Выбранный способ оплаты сейчас недоступен.');
}
function customer_order_payment_method_existing(string $id,array $fallback): array
{
    if($id==='')return $fallback;foreach(customer_payment_methods() as $method)if((string)$method['id']===$id)return $method;return $fallback;
}
function customer_order_snapshot(int $orderId): array
{
    $stmt=db()->prepare('SELECT order_number,status,payment_status,payment_method,total_amount FROM online_orders WHERE id=? LIMIT 1');$stmt->execute([$orderId]);$row=$stmt->fetch();if(!$row)throw new RuntimeException('Заказ не найден после оформления.');return $row;
}
function customer_order_create(array $data,?array $authenticatedCustomer=null): array
{
    ...
    $rawItems=$data['items']??null;if(!is_array($rawItems)||!$rawItems)throw new RuntimeException('Корзина пустая.');$lines=[];$baseIds=[];$baseUnits=0;
    ...
    foreach($lines as $line){$id=$line['product_id'];$qty=$line['quantity'];$p=$products[$id];$modifiers=customer_modifier_validate_selection($id,$line['modifier_option_ids']);$allUnits+=$qty*(1+count($modifiers));if($allUnits>100)throw new RuntimeException('Слишком много позиций и добавок в одном заказе.');$price=(float)$p['sale_price'];$lineTotal=$price*$qty;$total+=$lineTotal;$items[]=['external_id'=>(string)$id,'name'=>(string)$p['name'],'quantity'=>$qty,'unit_price'=>$price,'line_total'=>$lineTotal,'comment'=>null];foreach($modifiers as $m){$mPrice=(float)$m['price'];$mTotal=$mPrice*$qty;$total+=$mTotal;$items[]=['external_id'=>(string)$m['product_id'],'name'=>(string)$m['product_name'],'variant'=>(string)$m['label'],'quantity'=>$qty,'unit_price'=>$mPrice,'line_total'=>$mTotal,'comment'=>null];}}
    ...
    $publicId='customer-web-'.$clientOrderId;$orderNumber='W'.date('Hi').'-'.strtoupper(substr(hash('sha256',$clientOrderId),0,2));$initialPayment=$paymentMethod['id']==='sbp'?'pending':'unpaid';
    ...
    $result=online_orders_upsert_from_api($payload);$orderId=(int)$result['id'];customer_order_attach_product_identity($orderId);$locked=!$result['created']&&!empty($result['locked']);
    ...
    if($paymentMethod['id']==='sbp'&&round($total,2)<1){$paymentMethod=['id'=>'cash','label'=>'Подарок — к оплате 0 ₽','enabled'=>true,'online'=>false];}
    $payment=null;
    try{
        if($paymentMethod['id']==='sbp'){
    ...
    }catch(Throwable $e){if($paymentMethod['id']==='sbp'){db()->prepare("UPDATE online_orders SET status='cancelled',cancelled_at=NOW(),payment_status='failed',payment_method='sbp',payment_provider='yookassa_sbp' WHERE id=? AND status IN ('new','awaiting_payment')")->execute([$orderId]);try{customer_drink_loyalty_restore_online_order_reward($orderId,'Платёж не был создан, подарок возвращён');}catch(Throwable $restoreError){error_log('[Kapouch sixth drink restore] '.$restoreError->getMessage());}}throw $e;}

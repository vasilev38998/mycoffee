...
function cashflow_set_opening_balance(int $accountId,float $balance): void{if($accountId<=0||!is_finite($balance))throw new RuntimeException('Проверь денежный счёт и остаток.');$s=db()->prepare('UPDATE cash_flow_accounts SET opening_balance=? WHERE id=?');$s->execute([$balance,$accountId]);if($s->rowCount()===0){$q=db()->prepare('SELECT COUNT(*) FROM cash_flow_accounts WHERE id=?');$q->execute([$accountId]);if(!(int)$q->fetchColumn())throw new RuntimeException('Денежный счёт не найден.');}}
function cashflow_update_account(int $accountId,string $name,string $provider,bool $active): void{
    $name=trim($name);if($accountId<=0||$name==='')throw new RuntimeException('Укажи название денежного счёта.');$s=db()->prepare('UPDATE cash_flow_accounts SET name=?,provider=?,active=? WHERE id=?');$s->execute([mb_substr($name,0,160),mb_substr(trim($provider),0,80)?:null,$active?1:0,$accountId]);if($s->rowCount()===0){$q=db()->prepare('SELECT COUNT(*) FROM cash_flow_accounts WHERE id=?');$q->execute([$accountId]);if(!(int)$q->fetchColumn())throw new RuntimeException('Денежный счёт не найден.');}
}
function cashflow_validate_datetime(string $value): string{
    $value=trim($value);if($value==='')throw new RuntimeException('Укажите дату денежной операции.');$ts=strtotime($value);if($ts===false)throw new RuntimeException('Некорректная дата денежной операции.');return date('Y-m-d H:i:s',$ts);
}
function cashflow_add_entry(array $data): int{
    $accountId=(int)($data['account_id']??0);$direction=(string)($data['direction']??'');$type=(string)($data['entry_type']??'');$amount=(float)($data['amount']??0);$counter=(int)($data['counter_account_id']??0);
    if($accountId<=0||!in_array($direction,['in','out'],true)||!in_array($type,['sale','refund','expense','purchase','transfer','owner_in','owner_out','fee','other'],true)||!is_finite($amount)||$amount<=0)throw new RuntimeException('Некорректная денежная операция.');
    if($counter>0&&$counter===$accountId)throw new RuntimeException('Счёт и встречный счёт не должны совпадать.');
    $check=db()->prepare('SELECT id FROM cash_flow_accounts WHERE id=? LIMIT 1');$check->execute([$accountId]);if(!$check->fetchColumn())throw new RuntimeException('Денежный счёт не найден.');
    if($counter>0){$check->execute([$counter]);if(!$check->fetchColumn())throw new RuntimeException('Встречный денежный счёт не найден.');}
    $occurred=cashflow_validate_datetime((string)($data['occurred_at']??''));$u=current_user();$s=db()->prepare('INSERT INTO cash_flow_entries(occurred_at,account_id,direction,entry_type,amount,counter_account_id,source_type,source_id,category,description,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?)');
    $s->execute([$occurred,$accountId,$direction,$type,round($amount,2),$counter?:null,isset($data['source_type'])?mb_substr((string)$data['source_type'],0,80):null,isset($data['source_id'])?mb_substr((string)$data['source_id'],0,190):null,isset($data['category'])?mb_substr((string)$data['category'],0,120):null,isset($data['description'])?mb_substr((string)$data['description'],0,255):null,$u['id']??null]);return (int)db()->lastInsertId();
}
function cashflow_transfer(int $fromId,int $toId,float $amount,string $occurredAt,string $type='transfer',string $description=''): void{
    if($fromId<=0||$toId<=0||$fromId===$toId||$amount<=0||!is_finite($amount))throw new RuntimeException('Проверь счета и сумму перевода.');
...
function cashflow_manual_entry(int $accountId,string $direction,string $type,float $amount,string $occurredAt,string $category,string $description): int{
    if(!in_array($direction,['in','out'],true)||$amount<=0||!is_finite($amount))throw new RuntimeException('Некорректная операция.');return cashflow_add_entry(['occurred_at'=>$occurredAt,'account_id'=>$accountId,'direction'=>$direction,'entry_type'=>$type,'amount'=>$amount,'source_type'=>'manual','source_id'=>bin2hex(random_bytes(12)),'category'=>$category?:null,'description'=>$description?:null]);}
function cashflow_settle_electronic(float $gross,float $fee,int $bankAccountId,string $occurredAt,string $description='Зачисление безналичных продаж'): void{
    $pending=cashflow_account_by_name((string)app_setting('cashflow_electron_account_name','Безнал / ожидаемые поступления'));if(!$pending)throw new RuntimeException('Не найден счёт ожидаемых безналичных поступлений.');$bankStmt=db()->prepare("SELECT * FROM cash_flow_accounts WHERE id=? AND account_type='bank' AND active=1");$bankStmt->execute([$bankAccountId]);$bank=$bankStmt->fetch();if(!$bank)throw new RuntimeException('Выбери активный банковский счёт.');if(!is_finite($gross)||!is_finite($fee)||$gross<=0||$fee<0||$fee>$gross)throw new RuntimeException('Проверь сумму зачисления и комиссию.');$net=$gross-$fee;
...
function cashflow_sync_evotor_payments(int $limit=5000): array{
    $limit=max(1,min(20000,$limit));$cash=cashflow_account_by_name('Касса Эвотор');$electron=cashflow_account_by_name((string)app_setting('cashflow_electron_account_name','Безнал / ожидаемые поступления'));if(!$cash||!$electron)return ['processed'=>0,'cash'=>0,'electron'=>0];
    ...
    foreach($rows as $row){$doc=json_decode((string)$row['raw_json'],true);if(!is_array($doc))continue;$body=is_array($doc['body']??null)?$doc['body']:[];$refund=$row['document_type']==='PAYBACK';foreach(($body['payments']??[]) as $idx=>$p){if(!is_array($p))continue;$ptype=(string)($p['type']??'');if(!in_array($ptype,['CASH','ELECTRON'],true))continue;$amount=abs(cashflow_payment_amount($p));if(!is_finite($amount)||$amount<=0)continue;$account=$ptype==='CASH'?$cash:$electron;$direction=$refund?'out':'in';$entryType=$refund?'refund':'sale';$sourceId=(string)$row['evotor_document_id'].':'.$idx.':'.$ptype;
            $stmt=db()->prepare("INSERT INTO cash_flow_entries(occurred_at,account_id,direction,entry_type,amount,source_type,source_id,category,description) VALUES(?,?,?,?,?,'evotor_payment',?,'Продажи',?)");
            try{$stmt->execute([$row['close_date'],(int)$account['id'],$direction,$entryType,$amount,$sourceId,($ptype==='ELECTRON'?'Безнал':'Наличные').' · Эвотор '.$row['evotor_document_id']]);}
            catch(PDOException $e){if((int)($e->errorInfo[1]??0)===1062)continue;throw $e;}
            $result['processed']++;$result[$ptype==='CASH'?'cash':'electron']+=$refund?-$amount:$amount;
        }}return $result;
}

...
function cashflow_settle_acquiring(float $gross,float $fee,string $occurredAt,string $description='Зачисление эквайринга Сбер'): void{
    $acq=cashflow_account_by_name('Сбер Эквайринг');$bank=cashflow_account_by_name('Сбер Расчётный счёт');if(!$acq||!$bank)throw new RuntimeException('Не найдены счета Сбер Эквайринга.');if($gross<=0||$fee<0||$fee>$gross)throw new RuntimeException('Проверь сумму зачисления и комиссию.');$net=$gross-$fee;
    $pdo=db();$pdo->beginTransaction();try{$key='sber_settlement_'.bin2hex(random_bytes(8));if($net>0){cashflow_add_entry(['occurred_at'=>$occurredAt,'account_id'=>(int)$acq['id'],'direction'=>'out','entry_type'=>'transfer','amount'=>$net,'counter_account_id'=>(int)$bank['id'],'source_type'=>'sber_settlement','source_id'=>$key.':out','description'=>$description]);cashflow_add_entry(['occurred_at'=>$occurredAt,'account_id'=>(int)$bank['id'],'direction'=>'in','entry_type'=>'transfer','amount'=>$net,'counter_account_id'=>(int)$acq['id'],'source_type'=>'sber_settlement','source_id'=>$key.':in','description'=>$description]);}if($fee>0)cashflow_add_entry(['occurred_at'=>$occurredAt,'account_id'=>(int)$acq['id'],'direction'=>'out','entry_type'=>'fee','amount'=>$fee,'source_type'=>'sber_fee','source_id'=>$key.':fee','category'=>'Эквайринг','description'=>'Комиссия Сбер эквайринга']);$pdo->commit();}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}}
...
function cashflow_sync_evotor_payments(int $limit=5000): array{
    $cash=cashflow_account_by_name('Касса Эвотор');$acq=cashflow_account_by_name('Сбер Эквайринг');if(!$cash||!$acq)return ['processed'=>0,'cash'=>0,'electron'=>0];
...
function cashflow_acquiring_pending(): float{$a=cashflow_account_by_name('Сбер Эквайринг');return $a?cashflow_account_balance((int)$a['id']):0.0;}

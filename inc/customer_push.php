...
function customer_push_der_len(string $der,int &$offset): int{
    if($offset>=strlen($der))throw new RuntimeException('Некорректная VAPID-подпись.');$first=ord($der[$offset++]);if(($first&0x80)===0)return $first;$count=$first&0x7f;if($count<1||$count>4||$offset+$count>strlen($der))throw new RuntimeException('Некорректная VAPID-подпись.');$len=0;for($i=0;$i<$count;$i++)$len=($len<<8)|ord($der[$offset++]);return $len;
}
function customer_push_der_signature_to_raw(string $der): string{
    $o=0;if($der===''||ord($der[$o++])!==0x30)throw new RuntimeException('Некорректная VAPID-подпись.');customer_push_der_len($der,$o);
    if($o>=strlen($der)||ord($der[$o++])!==0x02)throw new RuntimeException('Некорректная VAPID-подпись.');$rl=customer_push_der_len($der,$o);if($rl<1||$o+$rl>strlen($der))throw new RuntimeException('Некорректная VAPID-подпись.');$r=substr($der,$o,$rl);$o+=$rl;
    if($o>=strlen($der)||ord($der[$o++])!==0x02)throw new RuntimeException('Некорректная VAPID-подпись.');$sl=customer_push_der_len($der,$o);if($sl<1||$o+$sl>strlen($der))throw new RuntimeException('Некорректная VAPID-подпись.');$s=substr($der,$o,$sl);
    $r=ltrim($r,"\0");$s=ltrim($s,"\0");if(strlen($r)>32||strlen($s)>32)throw new RuntimeException('Некорректная VAPID-подпись.');return str_pad($r,32,"\0",STR_PAD_LEFT).str_pad($s,32,"\0",STR_PAD_LEFT);
}
...
function customer_push_encrypt_payload(string $payload,string $clientKeyB64,string $authB64): string{
    $clientRaw=customer_push_b64url_decode($clientKeyB64);$auth=customer_push_b64url_decode($authB64);if($clientRaw===false||$auth===false||strlen($auth)<16||strlen($auth)>64)throw new RuntimeException('Некорректные ключи push-подписки.');
...
function customer_push_target_url(string $url): string{
    $url=trim($url);if($url==='')return '';
    if((str_starts_with($url,'./')||str_starts_with($url,'/')||str_starts_with($url,'#'))&&!str_starts_with($url,'//'))return mb_substr($url,0,500);
...
function customer_push_subscribe(int $customerId,array $subscription): void{
    ...
    if(strlen($endpoint)>2048||!is_array($keys))throw new RuntimeException('Некорректная push-подписка.');kapouch_public_https_target($endpoint);
    ...
    $p256raw=customer_push_b64url_decode($p256dh);$authRaw=customer_push_b64url_decode($auth);if($p256raw===false||strlen($p256raw)!==65||$p256raw[0]!=="\x04"||$authRaw===false||strlen($authRaw)<16||strlen($authRaw)>64)throw new RuntimeException('Некорректные ключи подписки.');
    if(strlen($p256dh)>255||strlen($auth)>255)throw new RuntimeException('Слишком длинные ключи push-подписки.');
...
function customer_push_enqueue(int $customerId,string $eventType,string $title,string $body,string $url,string $dedupeKey,?int $campaignId=null): bool{
    if($customerId<=0)return false;$target=customer_push_target_url($url);$stmt=db()->prepare("INSERT IGNORE INTO customer_push_queue(customer_id,campaign_id,event_type,title,body,target_url,dedupe_key,status) VALUES(?,?,?,?,?,?,?,'pending')");$stmt->execute([$customerId,$campaignId,mb_substr($eventType,0,40),mb_substr($title,0,120),mb_substr($body,0,500),$target!==''?$target:null,mb_substr($dedupeKey,0,190)]);return $stmt->rowCount()>0;
}
...
function customer_push_create_campaign(string $title,string $body,string $url,string $segment,?int $categoryId,?int $createdBy): array{
    $title=trim($title);$body=trim($body);if($title===''||$body==='')throw new RuntimeException('Укажите заголовок и текст push-уведомления.');if(!in_array($segment,['all','active30','inactive30','bonus','category'],true))$segment='all';if($segment!=='category')$categoryId=null;$target=customer_push_target_url($url);
...
function customer_push_process_queue(int $limit=20): array{
    ...
    try{
        $rows=$pdo->query("SELECT * FROM customer_push_queue WHERE status IN ('pending','failed') AND attempts<3 AND (next_attempt_at IS NULL OR next_attempt_at<=NOW()) ORDER BY id LIMIT {$limit} FOR UPDATE SKIP LOCKED")->fetchAll();
    ...
    foreach($rows as $row){
        ...
        foreach($subs as $sub){
            try{$r=customer_push_send_subscription($sub,$payload);}catch(Throwable $e){$r=['ok'=>false,'gone'=>false,'error'=>$e->getMessage(),'http'=>0];}

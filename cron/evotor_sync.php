...
$lock=fopen(sys_get_temp_dir().'/kapouch_evotor_sync.lock','c');
if(!$lock||!flock($lock,LOCK_EX|LOCK_NB)){echo '['.date('c')."] Evotor sync already running\n";exit(0);}
...
ensure_automatic_expense_tables();ensure_inventory_tables();ensure_cash_register_tables();ensure_control_tables();
